...
class TestWebauthN(common.DescopeTest):
    ...
    async def test_compose_signup_body(self):
        self.assertEqual(
    ...
    async def test_compose_signup_or_in_body(self):
        self.assertEqual(
    ...
    async def test_compose_update_start_body(self):
        self.assertEqual(
    ...
    async def test_compose_update_finish_body(self):
        self.assertEqual(
            WebAuthn._compose_update_finish_body("t01", "response01"),
            {"transactionId": "t01", "response": "response01"},
        )

    async def test_sign_up_start(self):
        webauthn = WebAuthn(
            Auth(
        ...
        with self.assertRaises(AuthException):
            await futu_await(webauthn.sign_up_start("", "https://example.com"))
        with self.assertRaises(AuthException):
            await futu_await(webauthn.sign_up_start("id1", ""))

        with mock_http_call(self.async_test, "post") as mock_post:
            mock_post.return_value.is_success = False
            with self.assertRaises(AuthException):
                await futu_await(webauthn.sign_up_start("id1", "https://example.com"))
        ...
        with mock_http_call(self.async_test, "post") as mock_post:
            ...
            res = await futu_await(webauthn.sign_up_start("id1", "https://example.com"))
    ...
    async def test_sign_up_finish(self):
        webauthn = WebAuthn(
            Auth(
                self.dummy_project_id, self.public_key_dict, async_mode=self.async_test
            )
        ...
        # Test failed flows
        with self.assertRaises(AuthException):
            await futu_await(webauthn.sign_up_finish("", "response01"))
        with self.assertRaises(AuthException):
            await futu_await(webauthn.sign_up_finish(None, "response01"))
        with self.assertRaises(AuthException):
            await futu_await(webauthn.sign_up_finish("t01", ""))
        with self.assertRaises(AuthException):
            await futu_await(webauthn.sign_up_finish("t01", None))
    ...
    async def test_sign_in_start(self):
        webauthn = WebAuthn(
            Auth(
                self.dummy_project_id, self.public_key_dict, async_mode=self.async_test
            )
        ...
        # Test failed flows
        with self.assertRaises(AuthException):
            await futu_await(webauthn.sign_in_start("", "https://example.com"))
        with self.assertRaises(AuthException):
            await futu_await(webauthn.sign_in_start("id", ""))
        ...
        with mock_http_call(self.async_test, "post") as mock_post:
            ...
            with self.assertRaises(AuthException):
                await futu_await(
                    webauthn.sign_in_start("id", "origin", LoginOptions(mfa=True)),
                )
        ...
        with mock_http_call(self.async_test, "post") as mock_post:
            ...
            my_mock_response.json.return_value = valid_response
            mock_post.return_value = my_mock_response
            res = await futu_await(webauthn.sign_in_start("id1", "https://example.com"))
    ...
    async def test_sign_in_start_with_login_options(self):
        webauthn = WebAuthn(
            Auth(
                self.dummy_project_id, self.public_key_dict, async_mode=self.async_test
            )
        ...
        with mock_http_call(self.async_test, "post") as mock_post:
            ...
            res = await futu_await(
                webauthn.sign_in_start("id1", "https://example.com", lo, "refresh")
            )
            expected_uri = f"{common.DEFAULT_BASE_URL}{EndpointsV1.sign_in_auth_webauthn_start_path}"
            mock_post.assert_called_with(
                expected_uri,
    ...
    async def test_sign_in_finish(self):
        webauthn = WebAuthn(
            Auth(
                self.dummy_project_id, self.public_key_dict, async_mode=self.async_test
            )
        ...
        # Test failed flows
        with self.assertRaises(AuthException):
            await futu_await(webauthn.sign_in_finish("", "response01"))
        with self.assertRaises(AuthException):
            await futu_await(webauthn.sign_in_finish(None, "response01"))
        with self.assertRaises(AuthException):
            await futu_await(webauthn.sign_in_finish("t01", ""))
        with self.assertRaises(AuthException):
            await futu_await(webauthn.sign_in_finish("t01", None))
    ...
    async def test_sign_up_or_in_start(self):
        ...
        # Test failed flows
        with self.assertRaises(AuthException):
            await futu_await(webauthn.sign_up_or_in_start("", "https://example.com"))
        with self.assertRaises(AuthException):
            await futu_await(webauthn.sign_up_or_in_start("id", ""))
        ...
        with mock_http_call(self.async_test, "post") as mock_post:
            ...
            mock_post.return_value = my_mock_response
            res = await futu_await(
                webauthn.sign_up_or_in_start("id1", "https://example.com")
            )
    ...
    async def test_update_start(self):
        ...
        # Test failed flows
        with self.assertRaises(AuthException):
            await futu_await(webauthn.update_start("", "", "https://example.com"))
        with self.assertRaises(AuthException):

            await futu_await(webauthn.update_start(None, "", "https://example.com"))
        with self.assertRaises(AuthException):
    ...
    async def test_update_finish(self):
        webauthn = WebAuthn(
            Auth(
                self.dummy_project_id, self.public_key_dict, async_mode=self.async_test
            )
        ...
        # Test failed flows
        with self.assertRaises(AuthException):
            await futu_await(webauthn.update_finish("", "response01"))
        with self.assertRaises(AuthException):
            await futu_await(webauthn.update_finish(None, "response01"))
        with self.assertRaises(AuthException):
            await futu_await(webauthn.update_finish("t01", ""))
        with self.assertRaises(AuthException):
            await futu_await(webauthn.update_finish("t01", None))
