...
class UnifiedClient(UnifiedClientBase):
    """
    Extends UnifiedClientBase (mode / attribute-delegation / invoke) with the
    mock_* helpers that patch the right HTTP layer per mode, so unit-test bodies
    run unchanged against both the sync and async clients.
    """
