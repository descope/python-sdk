...
class TestUser(common.DescopeTest):
    ...
    async def test_patch_with_status(self):
        ...
        for status in valid_statuses:
            with mock_http_call(self.async_test, "patch") as mock_patch:
                mock_patch.return_value.is_success = True
                mock_patch.return_value.json.return_value = {"user": {"id": "u1"}}
                resp = await futu_await(
                    self.client.mgmt.user.patch("id", status=status)
                )
                user = resp["user"]
                self.assertEqual(user["id"], "u1")
        ...
        with mock_http_call(self.async_test, "patch") as mock_patch:
            ...
            resp = await futu_await(
                self.client.mgmt.user.patch("id", display_name="test", status=None)
            )
            user = resp["user"]
            self.assertEqual(user["id"], "u1")
    ...
    async def test_patch_batch(self):
        ...
        with mock_http_call(self.async_test, "patch") as mock_patch:
            ...
            mock_patch.return_value.json.return_value = {
                "patchedUsers": [{"id": "u1"}, {"id": "u2"}, {"id": "u3"}],
                "failedUsers": [],
            }

            resp = await futu_await(self.client.mgmt.user.patch_batch(users))
        ...
        with mock_http_call(self.async_test, "patch") as mock_patch:
            ...
            mock_patch.return_value.json.return_value = {
                "patchedUsers": [{"id": "u1"}],
                "failedUsers": [
                    {"failure": "User not found", "user": {"loginId": "user2"}}
                ],
            }
        ...
        with mock_http_call(self.async_test, "patch") as mock_patch:
            ...
            with self.assertRaises(AuthException):
                await futu_await(
                    self.client.mgmt.user.patch_batch([UserObj(login_id="user1")])
                )
