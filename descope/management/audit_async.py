...
class AuditAsync(AuditBase, AsyncHTTPBase):
    """Async counterpart of Audit — all HTTP calls are coroutines."""
    ...
    async def search(
        ...
        response = await self._http.post(MgmtV1.audit_search, body=body)
        return {"audits": list(map(AuditAsync._convert_audit_record, response.json()["audits"]))}
    ...
    async def create_audit_webhook(self, name: str, url: Optional[str] = None, headers: Optional[dict] = None):
        """
        ...
        await self._http.post(MgmtV1.audit_webhook_set_path, body=body)
