...
def main():
    ...
    try:
        ...
        try:
            ...
            jwt_response = descope_client.exchange_access_key(access_key=access_key, login_options=loc)
            logging.info("exchange access key successfully")
            logging.info(f"jwt_response: {jwt_response}")

            permission_name = "TestPermission"
            permission_presented = descope_client.validate_permissions(jwt_response, [permission_name])
            logging.info(f"{permission_name} presented on the jwt: [{permission_presented}]")
            role_name = "TestRole"
